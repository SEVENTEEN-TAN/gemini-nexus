...
## 更新摘要
**已修改内容**
- 更新了“Background 环境中的 GeminiSessionManager”章节，移除了多账户轮换和重试逻辑的描述
- 更新了“AuthManager 上下文管理”章节，强调单账户模式下的行为
- 移除了架构总览和详细组件分析中的多账户轮换相关流程图
- 更新了“性能考虑”和“故障排除指南”中与多账户相关的建议
...
- background 中 GeminiSessionManager 如何与 Gemini API 交互、处理 AI 消息发送、上下文管理、单账户认证等高级能力
...
## 核心组件
- background/GeminiSessionManager：负责与 Gemini API 交互、上下文管理、工具调用执行与错误处理。
- background/AuthManager：负责认证上下文的加载、存储、模型变更检测与单账户管理。
...
- 关键流程
  - handleSendPrompt：取消上一次请求、构造文件列表、获取或刷新上下文、调用 sendGeminiMessage、解析工具调用并执行、更新上下文、返回结果。
...
BuildFiles --> GetContext["获取或刷新上下文"]
...
### AuthManager 上下文管理
- 功能
  - 初始化：从 chrome.storage.local 恢复上下文、模型。
  - 单账户管理：始终使用账户 '0'，不进行账户轮换。
...
- GeminiSessionManager 提供工具调用执行与健壮的错误处理，提升用户体验与稳定性。
