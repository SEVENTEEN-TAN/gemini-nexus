...
## 模型列表获取示例流程
新增模型列表获取功能，展示了如何通过双层通信架构获取AI模型列表。

### 流程步骤
1. **用户界面**: `ModelsController`调用`fetchModels`方法
2. **Sandbox发送**: 通过`postMessage`发送`FETCH_MODELS_LIST`消息
3. **Background接收**: `setupMessageListener`捕获`FETCH_MODELS_LIST`动作
4. **API调用**: 调用`getCachedModelsListAPI`获取模型列表
5. **响应处理**: 将模型数组作为响应返回
6. **错误处理**: 记录错误日志并在失败时返回空数组
7. **UI更新**: `ModelsController`接收响应并填充模型选择器

### 代码路径
- **Sandbox请求**: `sandbox/controllers/models_controller.js`中`fetchModels`方法
- **消息发送**: `window.parent.postMessage({ action: 'FETCH_MODELS_LIST', ... })`
- **Background处理**: `background/messages.js`中`FETCH_MODELS_LIST`分支
- **API实现**: `services/models_api.js`中`getCachedModelsListAPI`函数
- **UI更新**: `models_controller.js`中`populateModelSelects`方法

**本节来源**  
- [models_controller.js](file://sandbox/controllers/models_controller.js#L23-L78)
- [messages.js](file://background/messages.js#L88-L101)
- [models_api.js](file://services/models_api.js#L196-L219)